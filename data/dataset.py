...
def load_list_plus(dataset_list, data_root):
    ...
    for dataset_name in dataset_list:
        ...
        for depth in depth_files:
            ...
            depth_quality_list.append(depth_root.replace('/depth/', '/ssim2/') + depth[:-4]+'.png')

    return images, depths, labels, contours, depth_quality_list




class ImageData(data.Dataset):
    def __init__(self, dataset_list, data_root, transform, depth_transform,depth_quality_transform, mode, img_size=None, scale_size=None, t_transform=None, label_14_transform=None, label_28_transform=None, label_56_transform=None, label_112_transform=None):

        if mode == 'train':
            self.image_path, self.depth_path, self.label_path, self.contour_path, self.image_quality_map_path = load_list_plus(dataset_list, data_root)
        else:
            self.image_path, self.depth_path = load_test_list(dataset_list, data_root)

        self.transform = transform
        self.depth_transform = depth_transform
        self.depth_quality_transform = depth_quality_transform
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'train':
            ...
            depth_quality =  Image.open(self.image_quality_map_path[item]).convert('L')
            ...
            new_depth_quality =  trans.Scale((random_size, random_size))(depth_quality)
            ...
            if w != self.img_size and h != self.img_size:
                ...
                # add 10.3
                new_depth_quality = new_depth_quality.crop((x1, y1, x1 + self.img_size, y1 + self.img_size))
            ...
            if random.random() < 0.5:
                ...
                # add 10.3
                new_depth_quality = new_depth_quality.transpose(Image.FLIP_LEFT_RIGHT)
            ...
            new_depth_quality = self.depth_quality_transform(new_depth_quality)
            ...
            return new_img, new_depth, label_224, label_14, label_28, label_56, label_112, \
                   contour_224, contour_14, contour_28, contour_56, contour_112, new_depth_quality
...
def get_loader_pp(dataset_list, data_root, img_size, mode='train'):
    ...
    depth_transform = trans.Compose([transforms.Resize((img_size, img_size)), transforms.ToTensor()])


    dataset = ImageData(dataset_list, data_root, transform, depth_transform, depth_quality_transform=None,mode=mode)
